...
const THRESHOLD_WIDGET = new TransientWidget("smart-compaction-thresholds", {
...
function getThresholds(model: ModelLike): Thresholds {
  ...
  const compactThreshold = Math.max(
    0,
    contextWindow - responseReserve - promptReserve,
  );
  const warningGap = Math.min(
    WARNING_GAP_CAP,
...
class ThresholdOverlay {
  ...
  render(_width: number): string[] {
    ...
    return [
      ...
      row(
        ` ${this.theme.bold(this.theme.fg("accent", "Smart compaction thresholds"))}`,
      ),
      row(),
...
async function showThresholdOverlay(ctx: ExtensionCommandContext) {
  ...
  await ctx.ui.custom<void>(
    (_tui, theme, _keybindings, done) =>
      new ThresholdOverlay(theme, lines, () => done()),
    {
      overlay: true,
      overlayOptions: {
        anchor: "center",
...
function isOverflowMessage(message: unknown): boolean {
  ...
  return (
    /context(?:_|\s|-)?length(?:_|\s|-)?exceeded/i.test(errorMessage) ||
...
async function resolveSummarizer(
  ...
  for (const candidate of PREFERRED_SUMMARIZERS) {
    const model = ctx.modelRegistry.find(candidate.provider, candidate.id) as
      | ModelLike
      | undefined;
    if (!model) continue;
...
async function summarizeConversation(
  ...
  while (true) {
    const sections = current.map(
      (summary, index) => `### Partial summary ${index + 1}\n${summary}`,
    );
...
export default function smartCompaction(pi: ExtensionAPI) {
  ...
  pi.on("context", async (event) => {
    const messages = event.messages.filter(
      (message) => !isOverflowMessage(message),
    );
    return { messages };
  });
  ...
  pi.on("message_end", async (event, ctx) => {
    if (!isOverflowMessage(event.message)) {
      if (
        (event.message as { role?: unknown; stopReason?: unknown }).role ===
          "assistant" &&
    ...
    if (overflowRecoveryAttempted || compactionInFlight) {
      ctx.ui.notify(
        "Context overflow persisted after one smart-compaction retry.",
        "warning",
      );
      return;
    }
    ...
    if (!content) {
      ctx.ui.notify(
        "Context overflow detected, but the last user prompt could not be replayed.",
        "warning",
      );
      return;
    }
    ...
    ctx.compact({
      ...
      onError: (error) => {
        ...
        updateStatus(ctx);
        ctx.ui.notify(
          `Compaction failed after overflow: ${error.message}`,
          "error",
        );
      },
    });
  ...
  pi.on("input", async (event, ctx) => {
    ...
    if (queuedInputs.length >= COMPACTION_QUEUE_LIMIT) {
      ctx.ui.notify(
        "Compaction queue is full; wait for the current compaction to finish.",
        "warning",
      );
    ...
    queuedInputs.push({
      content:
        event.images && event.images.length > 0
          ...
          : event.text,
      fallbackText:
        event.images && event.images.length > 0 ? undefined : event.text,
    });
    if (compactionInFlight) {
  ...
  pi.on("session_before_compact", async (event, ctx) => {
    ...
    const details = getFileDetails(
      (preparation as { fileOps?: FileOps }).fileOps,
    );
    ctx.ui.notify(
